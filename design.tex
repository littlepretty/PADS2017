...
We can efficiently find the list of rules $\Delta[\alpha]$ that intersect with EC $\alpha$ using two data structures \kevin{what data structure? trie?}. Each is responsible for one of the two cases \cite{FindIntersectionWiki}.
\begin{itemize}
\item Case 1: Rule $\delta$ intersects $\alpha$ with its starting and/or end point in $\alpha$.
        ...
        We augment each value in $A$ with a pointer to the rule that it (\kevin{what does this ``it" refer to?}) belongs to.
        ...
        We then perform a linear search in the reduced list of ranges $A'$ (\kevin{$A'$ appears the first time, but not clear how to get $A'$}),
        check one by one if the interval that value $a\in A'$ belongs to intersects with $\alpha$ (\kevin{Please rephrase this sentence, it is hard to understand}).
...
that intersect with both the interval tree and the ordered list of the boundary values \kevin {value of what?}.
By mapping each rule to a unique binary ID of length $\log_2 M$,
we can encode $\Delta[\alpha]$ to a string $C_\alpha$ \kevin{of at most length $M\log_2 M$, putting
small binary IDs before large ones. (pls rephrase this sentence)}
We then use a hash table $H$ to group the mergeable ECs by hashing each EC $x$ to $C_x$.
The minimal size of ECs is the number of unique keys in $H$. Note that in the subsequent algorithmic designs, iterating through all (disjoint) ECs refers to iterating through the first ECs in each set $H[key]$.%, $\forall key \in H$.
