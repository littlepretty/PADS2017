...
We define \textbf{edge switches} as these switches whose port will be remapped to the port on
...
\Fn{\Traverse{$curr$, $src$, $snk$}} {
        \uIf {$curr$ is NOT visited} {
                $r \gets$ highest-priority rule on $curr$ that covers $x$\;
                \If {r is NULL or r.action is DROP} {
                        $snk \gets (curr, NULL)$\;
                        \GenRule{$x, src, snk$}\;
                        \KwRet\;
                }
                $next \gets topo[n][r.action.port]$\;
                \If {next $\not\in$ nodes} {
                        $snk \gets (curr, r.action.port)$\;
...
\ForEach{node $n \in nodes$} {
        \If {$node \in$ edge switches and NOT visited} {
                $src \gets (n, r.inport)$\;
                \Traverse{$n$, $src$, NULL}\;
        }
}
...
big switch at the end of forwarding graph traversal, whose procedure are described in
Algorithm~\ref{Alg:GenAllRules}.

\begin{algorithm}[h]
\DontPrintSemicolon
\KwData{$PortMap$ maps $port$ on $sw$ $\rightarrow port$ on $BS$ \newline
...
\Fn{\GenRule{$x, src, dst$}} {
        $r.match \gets x$\;
        \If {src.port not in PortMap[src.sw]} {
                $PortMap[src.sw][src.port] \gets global\_port++$\;
        }
        $r.inport = PortMap[src.sw][src.port]$\;
        \uIf {dst.port is NULL} {
                $r.action \gets drop\_action()$\;
        }
        \Else {
                \If {dst.port not in PortMap[dst.sw]} {
                        $PortMap[dst.sw][dst.port] \gets global\_port++$\;
                }
                $r.action \gets forward\_action()$\;
                $r.action.port \gets PortMap[dst.sw][dst.port]$\;
                
        }
}
\caption{Generate Rules for EC $x$ on Big Switch\label{Alg:GenAllRules}}
\end{algorithm}
