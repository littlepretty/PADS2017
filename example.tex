...
Our approach abstracts the network to one ``big switch" that has logically equivalent forwarding function.
...
four \textbf{disjoint} ECs based on the NW\_DST field as follows. Note that the matching field IN\_PORT cannot be used in identifying ECs, since it is not a packet-dependent, but topology-dependent filed.
...
The node in a forwarding graph represents a network device, and the directed edge represents how the network device forward the packets. 
The sink nodes, i.e., the red rectangle nodes in Figure~\ref{Fig:ExampleForwardingGraphs}, indicates the EC that this forwarding graph belongs to.
