...
While significantly \hl{reducing the model execution time and enabling the real-time simulation capability,}
...
and disjoint \hl{Equivalence Classes (ECes)} by analyzing the OpenFlow rules installed on the SDN devices.
\hl{We then create a graph model representing the forwarding behavior of each EC. By traversing those graphs, we finally construct the rules of the big-switch model to effectively preserve the original network's end-to-end forwarding behavior.}
